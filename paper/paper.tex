...
tabulated; they are supported on a small set of rational values, shared across the two
ensembles. We close with two numerical observations: the Gaussian-deficit ratios of
...
\section{Volume $1$ characterizes non-crossing pairings, with a $3/4$ gap}
...
$\sup f_r=f_r(r/2)\le f_5(5/2)=\tfrac{115}{192}<\tfrac23$ (the central values
$f_r(r/2)$ decrease in $r$), so any unit window has mass $<\tfrac23$.
\end{proof}
...
(\texttt{exact\_volume.py}); the largest run ($2k=12$, Toeplitz) takes under two
minutes on 14 cores.
